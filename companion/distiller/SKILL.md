...
description: 把一个联系人(同事/朋友/家人)从飞书聊天记录蒸馏成可对话的 AI 人格。产物写入 companion/personas/{slug}/, 供桌面陪伴助手作为 "蒸馏人格" 后端调用。
...
| 解析飞书导出 / Bot API 响应 | `Bash` → `python3 ${CLAUDE_SKILL_DIR}/tools/feishu_parser.py` |
| 写入 / 更新人格文件 | `Write` / `Edit` |

**基础目录**: 所有产物写到 `companion/personas/{slug}/` (相对 `pet/` 项目根)。
...
4. **隐私**: 所有原材料和人格只在本机 `companion/personas/{slug}/` 目录内, 不上传。
...
按 `${CLAUDE_SKILL_DIR}/prompts/intake.md` 只问 3 个问题:
1. 代号 (必填, 用作 slug)
2. 关系 + 基本信息 (一句话, 可跳过)
3. 性格画像 (一句话, 可跳过)
...
```bash
python3 ${CLAUDE_SKILL_DIR}/tools/feishu_parser.py \
  --file {path} \
...
**线 A (Shared Context)**: 参考 `${CLAUDE_SKILL_DIR}/prompts/memory_analyzer.md`
提取: 关系时间线、共同场景、交流节奏、inside jokes、工作/技术上下文、冲突模式、高光时刻、当前关系状态。

**线 B (Persona)**: 参考 `${CLAUDE_SKILL_DIR}/prompts/persona_analyzer.md`
提取: 说话风格、情绪反应、互动节奏、关系中的角色, 把用户给的标签按标签翻译表转为具体行为。

### Step 4: 生成预览

按 `${CLAUDE_SKILL_DIR}/prompts/memory_builder.md` 生成 Shared Context。
按 `${CLAUDE_SKILL_DIR}/prompts/persona_builder.md` 生成 5 层 Persona 草稿。
...
```bash
mkdir -p companion/personas/{slug}/versions
mkdir -p companion/personas/{slug}/sources
```

**2. 写入 `companion/personas/{slug}/memory.md`** (Shared Context 完整内容)。

**3. 写入 `companion/personas/{slug}/persona.md`** (5 层 Persona 完整内容)。

**4. 写入 `companion/personas/{slug}/meta.json`**:
...
**5. 复制原材料到 `companion/personas/{slug}/sources/`** (不包含在 git 内, 由 `.gitignore` 兜底)。

**6. 生成 `companion/personas/{slug}/SKILL.md`** —— 下游 backend 会把它整个当 system prompt:
...
3. 按 `${CLAUDE_SKILL_DIR}/prompts/merger.md` 分析增量。
...
1. 按 `${CLAUDE_SKILL_DIR}/prompts/correction_handler.md` 识别纠正。
...
```bash
ls companion/personas/ | while read d; do
  [ -f "companion/personas/$d/meta.json" ] && echo "- $d"
done
```

`/delete-contact {slug}`: 确认后 `rm -rf companion/personas/{slug}`。
