Create a single horizontal sprite strip for the Codex app digital pet `xinxin` in the state `idle`.
...
Xinxin personality direction: make this a calm warm baseline, with a tiny head tilt, soft blink, subtle breathing, a gentle smiling face, and a barely visible sway in the long hair and layered skirt. She loves singing, so the loop may feel like quiet humming through a tiny soft mouth-shape change, but keep it low-distraction and do not add music notes, text, symbols, or speech bubbles.
