Create a single horizontal sprite strip for the Codex app digital pet `xinxin` in the state `running`.
...
Xinxin personality direction: make this an active helpful loop, as if she is quietly cheering the task forward with a happy little melody. Use a small focused nod, tiny hand movement near the bow or chest, lively hair/skirt bounce, smiling eyes, and a determined kind expression. Do not show literal running, jogging, sprinting, travel, tools, papers, screens, music notes, symbols, or text.
