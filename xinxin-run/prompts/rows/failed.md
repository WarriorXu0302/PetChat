Create a single horizontal sprite strip for the Codex app digital pet `xinxin` in the state `failed`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: A tiny elegant human-like chibi digital pet named Xinxin based on the approved sprite reference: long dark wavy hair, large soft sparkling eyes, gentle smile, pink strapless scarf-style top with a side bow, layered warm-white ruffled skirt, small gold necklace, tiny warm-white shoes, graceful and lively posture..

Style contract: Codex digital pet sprite style: pixel-art-adjacent low-resolution mascot sprite, compact chibi proportions, chunky whole-body silhouette, thick dark 1-2 px outline, visible stepped/pixel edges, limited palette, flat cel shading with at most one small highlight and one shadow step, simple readable face, tiny limbs, and no detail that disappears at 192x208. Avoid polished illustration, painterly rendering, anime key art, 3D render, vector app-icon polish, glossy lighting, soft gradients, realistic fur or material texture, anti-aliased high-detail edges, and complex tiny accessories. Additional user style notes: Preserve the approved base identity exactly: same long hair silhouette, pink-and-white outfit, side bow, layered skirt, soft face, compact chibi proportions, thick dark pixel outline, crisp stepped edges, limited blush pink warm white charcoal brown gold palette, flat cel shading, animation-friendly details, no text, no scenery, no shadows, no glow..
...
Xinxin personality direction: make this a tender self-comforting reaction rather than dramatic despair. Use a small slumped posture, softened eyes, a tiny attached tear or attached star only if it stays on the silhouette, then a gentle recovering frame with a faint reassuring smile, as if she is softly humming comfort to herself. No text, no speech bubble, no red X, no music notes, no detached effects.
...
- Use a perfectly flat pure user-selected #00FFFF chroma-key background across the whole image.
...
- Do not use #00FFFF, pure user-selected, or colors close to that chroma key in the pet, props, highlights, shadows, motion marks, dust, landing marks, or effects.
